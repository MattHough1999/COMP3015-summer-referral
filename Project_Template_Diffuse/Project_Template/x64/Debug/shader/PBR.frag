...
{
    vec3 f0 = vec3(0.04);
    if(Material.Metal) {f0 = Material.Color;}
...
vec3 microfacetModel(int lightIdx, vec3 position, vec3 n)
{
    ...
    if(!Material.Metal){diffuseBrdf = Material.Color;}
    vec3 l = vec3(0.0), lightI = Light[lightIdx].L;

    if(Light[lightIdx].Position.w == 0.0){l = normalize(Light[lightIdx].Position.xyz);}
    else
    {
